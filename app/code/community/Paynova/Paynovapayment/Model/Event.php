 ...
 */
class Paynova_Paynovapayment_Model_Event
{
    ...
    public function cancelEvent() {
        try {
            ...
            return Mage::helper('paynovapayment')->__('The order has been canceled.');
        } catch (Mage_Core_Exception $e) {
            return $e->getMessage();
        } catch(Exception $e) {
            Mage::logException($e);
     ...
     * @return int
     */
    public function successEvent($t = true){
        $params = $this->_validateEventData($t);
    ...
    protected function _validateEventData($fullCheck = true)
    {
        ...
        if (0 !== strpos($this->_order->getPayment()->getMethodInstance()->getCode(), 'paynovapayment_')) {
        ...
        if ($fullCheck) {
            ...
            $secretKey = Mage::getStoreConfig(
                Paynova_Paynovapayment_Helper_Data::XML_PATH_SECRET_KEY,
