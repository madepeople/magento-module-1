 ...
 */
abstract class Paynova_Paso_Model_Abstract extends Mage_Payment_Model_Method_Abstract
{
    /**
     * unique internal payment method identifier
     */
    protected $_code = 'paso_abstract';

    protected $_formBlockType = 'paso/form';
    protected $_infoBlockType = 'paso/info';
 ...
 */
     ...
     */
    public function setCurlCall($data, $address){

        if ($data) {

            Mage::helper('paso')->log($address);
            Mage::helper('paso')->log($data);
            $data = json_encode($data);

            $apiUrl=Mage::getStoreConfig('paso/settings/api_live_url');
            $apiTest=Mage::getStoreConfig('paso/settings/api_test_url');
            $apiMode=Mage::getStoreConfig('paso/settings/api_mode');
            $password=Mage::helper('core')->decrypt(Mage::getStoreConfig('paso/settings/password'));
            $username=Mage::getStoreConfig('paso/settings/merchant_id');
            ...
            $output = curl_exec($ch);
            Mage::helper('paso')->log(json_decode($output));
   ...
   /**
    ...
    {
        return Mage::getUrl('paso/processing/payment');
    }
     ...
     * @return Paynova_Paso_Model_Abstract
     */
    public function captureInvoice(Varien_Object $payment)
    {
        ...
        foreach ($items as $itemId => $item)
        {
            ...
            $res['lineItems'][$itemId]['articleNumber'] = $order_item->getId();
    ...
    public function invoicing($order, $send_email = false)
    {
       try {
            ...
            $autofinalizemail = Mage::getStoreConfig('paso/settings/autofinalizemail');
    ...
    public function getFormFields()
    {
        ...
        $params = array(
            ...
            'partner'               => 'magento',
            'pay_to_email'          => Mage::getStoreConfig(Paynova_Paso_Helper_Data::XML_PATH_EMAIL),
            'transaction_id'        => $order_id,
            'return_url'            => Mage::getUrl('paso/processing/success', array('transaction_id' => $order_id)),
            'cancel_url'            => Mage::getUrl('paso/processing/cancel', array('transaction_id' => $order_id)),
            'status_url'            => Mage::getUrl('paso/processing/status'),
            ...
            'detail1_description'   => Mage::helper('paso')->__('Order ID'),
